...
export function SharedLeadForm({
  ...
  if (submitted) {
    return <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-7 text-center text-emerald-800">
      <CheckCircle2 className="mx-auto mb-3" size={42} />
      <h3 className="text-xl font-extrabold">تم إرسال نتيجة التقييم بنجاح</h3>
      <p className="mt-2">أصبحت النتيجة متاحة لفرق الشركات المسجلة في منصة نبراس CRM.</p>
    </div>;
  }
  ...
  return <form action={submit} className="rounded-3xl bg-[#002b22] p-6 text-right text-white sm:p-8">
    ...
    <p className="mt-3 text-sm leading-7 text-white/65">ستظهر النتيجة كسجل مشترك واحد داخل CRM لجميع الشركات المسجلة.</p>
