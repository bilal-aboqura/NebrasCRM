...
function AssessmentContent() {
  ...
  useEffect(() => {
    if (typeParam === "general" || typeParam === "dental") setFacilityType(typeParam);
  ...
  }, [facilityId, typeParam]);

  return <main className="min-h-screen bg-nebras-cream text-[#102820]">
    <div className="bg-nebras-green text-white print:hidden">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-center gap-x-6 gap-y-2 px-5 py-2.5 text-xs sm:justify-between sm:text-sm">
        <div className="flex flex-wrap items-center justify-center gap-5">
          <a href="tel:+966502658846" dir="ltr" className="inline-flex items-center gap-1.5 hover:text-nebras-gold"><Phone size={15} /> +966 50 265 8846</a>
          <a href="mailto:NEBRASGOO@GMAIL.COM" dir="ltr" className="inline-flex items-center gap-1.5 hover:text-nebras-gold"><Mail size={15} /> NEBRASGOO@GMAIL.COM</a>
        </div>
        <p className="font-bold text-nebras-gold">منصة تقييم ذاتي مبنية على أدلة سباهي</p>
      </div>
    </div>
    ...
    <section className="relative overflow-hidden bg-gradient-to-bl from-nebras-green via-[#075540] to-nebras-green text-white">
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-5 py-16 lg:grid-cols-[1.15fr_.85fr] lg:px-8 lg:py-24">
        <div>
          ...
          <h1 className="mt-6 text-4xl font-extrabold leading-tight sm:text-5xl lg:text-6xl">قيّم جاهزية منشأتك<span className="block text-nebras-gold">لاعتماد سباهي</span></h1>
          <p className="mt-6 max-w-2xl text-lg leading-9 text-white/75">أداة عملية للمجمعات الطبية العامة ومجمعات الأسنان، مبنية على فصول ومعايير أدلة سباهي وتساعدك على معرفة الفجوات والتوصيات.</p>
          <div className="mt-8 flex flex-wrap gap-3 print:hidden">
            <a href="#start" className="inline-flex items-center gap-2 rounded-full bg-nebras-gold px-7 py-3.5 font-extrabold text-nebras-green shadow-lg">ابدأ التقييم الآن <ArrowDown size={18} /></a>
            <button onClick={reset} className="inline-flex items-center gap-2 rounded-full border border-white/30 px-7 py-3.5 font-bold hover:bg-white/10"><RotateCcw size={18} /> تصفير التقييم</button>
          </div>
        </div>
        <div className="mx-auto w-full max-w-sm rounded-[2rem] border border-white/15 bg-white p-8 text-center text-nebras-green shadow-2xl">
          <div className="mx-auto grid size-48 place-items-center rounded-full p-3" style={{ background: `conic-gradient(#c4a35a ${score * 3.6}deg, #eee4d2 0deg)` }}>
            <div className="grid size-full place-items-center rounded-full bg-white"><strong className="text-5xl font-black">{score}%</strong></div>
          </div>
          <h2 className="mt-6 text-xl font-extrabold">مؤشر الجاهزية العام</h2>
          <p className="mt-2 leading-7 text-slate-600">{scoreBreakdown.answeredCount ? scoreBreakdown.tierDescription : "اختر نوع المنشأة وابدأ الإجابة على البنود."}</p>
        </div>
      </div>
    </section>
    ...
    <section id="start" className="scroll-mt-24 border-b border-[#e7ddc9] bg-white py-12 print:hidden">
      <div className="mx-auto max-w-5xl px-5">
        <div className="mb-7 flex items-center gap-4"><span className="h-px flex-1 bg-[#e7ddc9]" /><h2 className="text-2xl font-extrabold text-nebras-green">اختر نوع المنشأة</h2><span className="h-px flex-1 bg-[#e7ddc9]" /></div>
        <FacilityTypeSelector currentType={state.facilityType} onChange={setFacilityType} hasAnswers={scoreBreakdown.answeredCount > 0} />
        {prelinkedFacility && <div className="mt-5 rounded-xl border border-blue-100 bg-blue-50 p-4 text-blue-800"><strong>تقييم مرتبط بمنشأة:</strong> {prelinkedFacility.name_ar}</div>}
      </div>
    </section>

    {!state.showReport && <section id="dashboard" className="scroll-mt-24 mx-auto grid max-w-7xl gap-7 px-5 py-12 lg:grid-cols-[320px_1fr] lg:items-start lg:px-8">
      <ScoringSidebar scoreBreakdown={scoreBreakdown} onReset={reset} onGenerateReport={() => setShowReport(true)} />
      <AssessmentPanel facilityType={state.facilityType} answers={state.answers} notes={state.notes} activeChapterFilter={state.activeChapterFilter} setAnswer={setAnswer} setNote={setNote} setChapterFilter={setChapterFilter} />
    </section>}

    {state.showReport && <section id="report" className="scroll-mt-24 mx-auto max-w-6xl px-5 py-12 lg:px-8">
      <GapReportSection scoreBreakdown={scoreBreakdown} facilityType={state.facilityType} answers={state.answers} notes={state.notes} onBack={() => setShowReport(false)} />
    </section>}

    <section className="border-y border-[#e7ddc9] bg-[#f6f2e9] py-12 print:hidden">
      <div className="mx-auto max-w-5xl px-5">
        <h2 className="flex items-center gap-2 text-xl font-extrabold text-nebras-green"><ShieldCheck className="text-nebras-gold" /> ملاحظات مهمة</h2>
        <ul className="mt-5 grid gap-3 leading-8 text-slate-600">
          <li>هذه الأداة للتقييم الذاتي الأولي وليست بديلاً عن دليل سباهي الرسمي أو زيارة التقييم الميدانية.</li>
          <li>تمت صياغة البنود بشكل مختصر ومنظم للاستخدام العملي دون نسخ نصوص الدليل كاملة.</li>
          <li>نتيجة التقييم المرسلة تظهر كسجل مشترك واحد لجميع الشركات داخل CRM.</li>
        </ul>
      </div>
    </section>
...
export default function AssessmentPage() {
  return <Suspense fallback={<div className="grid min-h-screen place-items-center bg-nebras-cream font-bold text-nebras-green">جارٍ تحميل أداة التقييم...</div>}><AssessmentContent /></Suspense>;
}
