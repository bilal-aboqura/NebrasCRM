  ...
  return (
    <section dir="rtl" className="space-y-7">
      <header>
        ...
        <h1 className="mt-1 text-3xl font-extrabold text-nebras-green">
          أهلاً، {context.fullName || context.email}
        </h1>
        <p className="mt-2 text-slate-600">
          نظرة شاملة على العمل في <strong>{context.companyName}</strong>
        </p>
      </header>
      <DashboardClient
        data={data}
        teamPerformance={teamPerformance}
        onPeriodChange={changePerformancePeriod}
      />
    </section>
  );
}
