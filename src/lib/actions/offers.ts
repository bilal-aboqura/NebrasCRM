...
function mapOffer(row: Record<string, any>): Offer {
  ...
  return {
    ...
    discountValue: Number(row.discount_value), discountAmount: Number(row.discount_amount),
    taxRate: Number(row.tax_rate), taxAmount: Number(row.tax_amount), grandTotal: Number(row.grand_total),
