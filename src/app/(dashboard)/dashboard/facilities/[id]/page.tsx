...
export default async function FacilityDetailPage({ params, searchParams }: { params: { id: string }; searchParams?: { callsPage?: string; archivedCalls?: string } }) {
  ...
  const [activity, options, contactsResult, archivedResult, followUpsResult, followUpOptions, callLogsResult, offersResult, archivedOffersResult, offerOptions, contractsResult, contractOptions] = await Promise.all([
    ...
    getFacilityOffers(params.id), detail.canManage ? getFacilityOffers(params.id, true) : Promise.resolve({ success: true as const, data: [] }), detail.data.is_active ? getOfferOptions(params.id) : Promise.resolve({ success: true as const, data: { contacts: [], canManage: detail.canManage } }),
  ...
  return <section className="space-y-6" dir="rtl">
    ...
    {offersResult.success && offerOptions.success ? <OffersSection facilityId={facility.id} facilityName={facility.name_ar} facilityStatus={facility.status} offers={offersResult.data} archivedOffers={archivedOffersResult.success ? archivedOffersResult.data : []} contacts={offerOptions.data.contacts} canEdit={Boolean(facility.is_active)} canManage={detail.canManage && Boolean(facility.is_active)} /> : <article className="rounded-2xl bg-white p-6 text-red-700 shadow-sm">{offersResult.success ? (offerOptions.success ? "" : offerOptions.error.message) : offersResult.error.message}</article>}
