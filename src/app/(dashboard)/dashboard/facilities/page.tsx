  ...
  return (
    <section className="space-y-6" dir="rtl">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="font-bold text-nebras-gold">إدارة العملاء</p>
          <h1 className="text-3xl font-extrabold text-nebras-green">قائمة المنشآت</h1>
          <p className="mt-2 text-slate-600">{meta.total} منشأة ضمن النطاق المسموح لك</p>
        </div>

        <div className="flex flex-wrap gap-2">
          <ExportButton endpoint="/api/facilities/export" params={searchParams} />
          {options.canAssign && <ImportModal />}
          <FacilityForm {...options} />
        </div>
      </div>
      ...
      <form className="grid gap-3 rounded-2xl bg-white p-4 shadow-sm sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
        ...
        <select name="status" defaultValue={param("status")} className={control} aria-label="الحالة">
        ...
        <select name="type" defaultValue={param("type")} className={control} aria-label="النوع">
        ...
        {options.canAssign && (
          <select name="owner" defaultValue={param("owner")} className={control} aria-label="المسؤول">
            <option value="">كل المسؤولين</option>
            {options.owners.map((owner) => (
              <option key={owner.id} value={owner.id}>
                {owner.display_name}
                {owner.status !== "active" ? " (غير نشط)" : ""}
              </option>
            ))}
          </select>
        )}
      ...
      <div className="overflow-x-auto rounded-2xl bg-white shadow-sm">
        <table className="w-full min-w-[850px] text-right">
          ...
          <FacilitiesClient
            records={records as FacilityListRecord[]}
            typeLabels={FACILITY_TYPE_LABELS}
            statusLabels={FACILITY_STATUS_LABELS}
          />
        </table>
      </div>

      <nav className="flex items-center justify-center gap-4" aria-label="صفحات المنشآت">
        <Link
          aria-disabled={meta.page <= 1}
          href={`?${queryFor(Math.max(1, meta.page - 1))}`}
          className={`rounded-lg border p-2 ${meta.page <= 1 ? "pointer-events-none opacity-40" : ""}`}
        >
          <ChevronRight />
        </Link>
        ...
        <Link
          ...
          href={`?${queryFor(Math.min(meta.pages, meta.page + 1))}`}
          className={`rounded-lg border p-2 ${meta.page >= meta.pages ? "pointer-events-none opacity-40" : ""}`}
        >
          <ChevronLeft />
        </Link>
      </nav>
