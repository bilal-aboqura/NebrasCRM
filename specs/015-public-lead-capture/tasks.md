...
- [X] T001 Configure default target company ID `DEFAULT_LEAD_COMPANY_ID=company-a` in local environment file [.env.local](file:///f:/CodingProjects/NebrasCRM/.env.local)
...
- [X] T002 Create client IP rate limiter utility in [src/lib/rate-limit/memory.ts](file:///f:/CodingProjects/NebrasCRM/src/lib/rate-limit/memory.ts)
- [X] T003 [P] Add `lead_source` optional field to the `Facility` type declaration in [src/lib/types/domain.ts](file:///f:/CodingProjects/NebrasCRM/src/lib/types/domain.ts)
