                  ...
                  {standard.items.map((item) => (
                    <div key={item.code} className="p-4 transition-colors hover:bg-white/70 sm:p-5">
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        ...
                        <div className="w-full shrink-0 print:hidden lg:w-48">
                          <select
                            value={answers[item.code] || ""}
                            onChange={(event) => setAnswer(item.code, event.target.value as AnswerValue)}
                          ...
                          >
                            ...
                            <option value="0.5">متوفر جزئيًا (0.5)</option>
                            <option value="0">غير متوفر (0)</option>
                            <option value="na">غير منطبق (N/A)</option>
                          </select>
                        </div>

                        <div className="hidden w-full text-sm print:block">
                          <strong>التقييم: </strong>
                          {answers[item.code] === "1"
                            ? "متوفر ومطبق بالكامل (1)"
                            : answers[item.code] === "0.5"
                              ? "متوفر جزئيًا (0.5)"
                              : answers[item.code] === "0"
                                ? "غير متوفر (0)"
                                : answers[item.code] === "na"
                                  ? "غير منطبق (N/A)"
                                  : "لم يتم التقييم"}
                        </div>
                      </div>
                      ...
                      <div className="mt-4 print:hidden">
                        <div className="flex items-start gap-2">
                          ...
                          <textarea
                            ...
                            className="w-full resize-none rounded-lg border border-gray-200 bg-white p-2 text-sm text-gray-700 outline-none focus:border-nebras-green focus:ring-1 focus:ring-nebras-green"
                            rows={2}
                          />
                        </div>
                      </div>
                      ...
                      {notes[item.code] && (
                        <div className="mt-2 hidden rounded bg-gray-50 p-2 text-sm text-gray-600 print:block">
