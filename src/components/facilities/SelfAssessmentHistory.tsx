      ...
      <div className="mb-4 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <div className={!canManage ? "ml-auto" : ""}>
          <a
            ...
            className="flex items-center gap-2 rounded-lg bg-nebras-green px-4 py-2 text-sm font-bold text-white transition-colors hover:bg-[#208f60]"
          >
            <FileText size={16} />
            بدء تقييم جديد
          </a>
      ...
      {assessments.length === 0 ? (
        ...
        assessments.map((assessment) => (
          <div
            ...
            className={`cursor-pointer rounded-xl border p-5 shadow-sm transition-shadow hover:shadow-md ${!assessment.isActive ? "border-gray-200 bg-gray-50" : "border-gray-100 bg-white"}`}
          >
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
              <div>
                <div className="mb-1 flex items-center gap-3">
                  <span className="text-lg font-bold text-gray-800">{assessment.overallScore}%</span>
                  <span className={`rounded px-2 py-1 text-xs font-bold ${!assessment.isActive ? "bg-gray-200 text-gray-600" : getTierColor(assessment.readinessTier)}`}>
              ...
              <div className="flex items-center gap-3">
                {assessment.previousScore !== undefined && assessment.delta !== undefined && (
                  <div className="flex items-center gap-2 rounded-lg bg-slate-50 px-4 py-2 text-sm">
                    ...
                    <span className={`ml-1 font-bold dir-ltr ${assessment.delta > 0 ? "text-green-600" : assessment.delta < 0 ? "text-red-600" : "text-gray-500"}`}>
                ...
                {canManage && !assessment.isActive && (
                  <button
                    ...
                    className="flex items-center justify-center rounded-lg bg-green-50 p-2 text-green-600 transition-colors hover:bg-green-100"
