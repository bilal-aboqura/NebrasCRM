...
export function UserInviteForm({ isSuperAdmin, companies }: { isSuperAdmin: boolean; companies: { id: string; name_ar: string }[] }) {
  ...
  return <form action={action} className="grid gap-4 rounded-2xl bg-white p-6 shadow-sm md:grid-cols-2">
    ...
    {isSuperAdmin && <label className="space-y-2"><span>الشركة</span><select name="company_id" className="w-full rounded-lg border px-3 py-2"><option value="">بدون شركة (لمدير النظام فقط)</option>{companies.map((company) => <option key={company.id} value={company.id}>{company.name_ar}</option>)}</select></label>}
