...
- [ ] T019 [US3] Add the outcome-aware checkbox "Ø¥ØªÙ…Ø§Ù… Ø§Ù„Ù…ØªØ§Ø¨Ø¹Ø© Ø§Ù„Ù…Ø±ØªØ¨Ø·Ø©" (Mark linked follow-up as completed) and default toggle state calculations to the `LogCommunicationModal` in `src/components/facilities/LogCommunicationModal.tsx`.
...
- [ ] T028 [US5] Add the edit forms and "ØªØ¹Ø¯ÙŠÙ„" (Edit) and "Ø£Ø±Ø´ÙØ©" (Archive) actions to `CallLogsSection` in `src/components/facilities/CallLogsSection.tsx` with client-side 24-hour lock button controls.
